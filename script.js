...
function initContactForm() {
  ...
  form.addEventListener('submit', e => {
    ...
    const msg = {
      ...
      email: inputs[2].value, interest: inputs[3].value,
      message: inputs[4].value, date: new Date().toLocaleDateString()
    };
    messages.push(msg);
    saveMessages();
...
document.getElementById('customerLoginForm').addEventListener('submit', e => {
  ...
  const user = users.find(u => u.email === email && u.password === pass && !u.blocked);
  if (user) {
...
document.getElementById('registerForm').addEventListener('submit', e => {
  ...
  const user = { id: Date.now(), name, email, phone, password: pass, blocked: false, date: new Date().toLocaleDateString() };
  users.push(user);
  saveUsers();
  customerLoggedIn = user;
  closeModal('loginModal');
...
function openAdminDashboard() {
  ...
  buildAdminTopbar();        // ← premium topbar
  syncNavbarDropdowns();     // ← sync live navbar with saved customOptions
}
...
function renderAdminUsers() {
  ...
  if (!users.length) { tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;color:var(--text-muted)">No registered users yet</td></tr>'; return; }
  tbody.innerHTML = users.map(u => `
    <tr>
      <td style="color:var(--text)">${u.name}</td>
      <td>${u.email}</td>
      <td>${u.phone || '-'}</td>
      <td><span style="color:${u.blocked ? '#ef4444' : '#22c55e'}">${u.blocked ? 'Blocked' : 'Active'}</span></td>
      <td>
        <div class="action-btns">
...
function renderAdminMessages() {
  ...
  if (!messages.length) {
    const empty = '<tr><td colspan="7" style="text-align:center;color:var(--text-muted)">No messages yet</td></tr>';
    tbody.innerHTML = empty;
    enquiryTbody.innerHTML = '<tr><td colspan="6" style="text-align:center;color:var(--text-muted)">No enquiries yet</td></tr>';
    return;
  }
  // Newest first (reverse), with serial #1 = latest
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  applyAllSettings();
  initReveal();
  syncNavbarDropdowns();   // ← apply any saved custom options to navbar on load
  ...
  // Re-init observer for newly added reveals
